...
class TestUtils_process_image(unittest.TestCase):
    ...
    def test_detect_feature(self):
        ...
        file_in = 'out/result.tif'
        file_out = 'out/result.csv'
        result = None
        frames = utils.read_tif(file_in)
        results = utils.detect_features(frames)
        self.assertIsNotNone(frames)
        utils.write_csv(results, file_out)
#         self.assertEqual(file out value, expected value)
        file = open(file_out)
        self.assertIsNotNone(file)
        file.close()
